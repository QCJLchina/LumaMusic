...
int main(){
 try {
    for(unsigned channels:{1,2,6,8}){
        ...
        for(size_t f=0;f<1024;++f)for(unsigned c=0;c<channels;++c){int32_t v=(int32_t)std::lrint(output[f*channels+c]*8388608.f);check(((v>>16)&255)==(f%2?0xfa:0x05));check(((v>>8)&255)==input[f*channels*2+c]);check((v&255)==input[(f*2+1)*channels+c]);}
        check(!marker);
    }
